...
func NewHttp(h *http.Client, apiKey, apiSecret string) *Bittrex {
	client := NewClientHttp(h, apiKey, apiSecret)
	return &Bittrex{client}
}
...
func (b *Bittrex) GetNewCandles(market, LastEpoch string) (candles []Candle, err error) {
	r, err := b.client.do("GET", "https://www.bittrex.com/Api/v2.0/pub/market/GetTicks?MarketName="+strings.ToUpper(market)+"&tickInterval="+LastEpoch+"&_=1503075320403", "", false)
	if err != nil {
		return
	}
...
"Currency":"DGD",
"Address":"0xbdb6ef37fd9a75078e7f461ae3a80f4b9fad5f65",
"BaseAddress":"0xfbb1b73c4f0bda4f67dca266ce6ef42f520fbb98"}}
*/
func (b *Bittrex) GenerateDepositAddress(currency string) (address Address, err error) {
	r, err := b.client.do("POST", "https://bittrex.com/api/v2.0/auth/balance/GenerateDepositAddress", "currencyName="+strings.ToUpper(currency), true)
	if err != nil {
		return
	}
	var response jsonResponse
...
func (b *Bittrex) WithdrawPaymentID(address, paymentID, currency string, quantity float64) (withdrawUuid string, err error) {

	paymentString := ""
	if strings.Compare("", paymentID) != 0 {
		paymentString = "&paymentid=" + paymentID
	}
	r, err := b.client.do("GET", "account/withdraw?currency="+
		strings.ToUpper(currency)+
		"&quantity="+strconv.FormatFloat(quantity, 'f', 8, 64)+
		"&address="+address+paymentString, "", true,
	)
	if err != nil {
		return
	}
...
// GetWithdrawalHistory is used to retrieve your withdrawal history
// currency string a string literal for the currency (ie. BTC). If set to "all", will return for all currencies
func (b *Bittrex) GetPendingWithdrawals(currency string) (withdrawals []Withdrawal, err error) {
	ressource := "https://www.bittrex.com/api/v2.0/auth/balance/GetPendingWithdrawals"
